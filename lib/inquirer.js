...
module.exports = {
	selectTest: function(tests) {
		const questions = [
			...
			},
			{
				...
				when: function(ctx) {
					var exists = tests.map((t) => t.number).indexOf(ctx.number) > -1;
					return !exists;
				},
				validate: function( value ) {
					if (value.length) {
		...
		return inquirer.prompt(questions).then((answer) => {
			const test = tests.filter((t) => t.number == answer.number)[0] || answer;
	...
	makeTestReport: function(existingReports, requirements) {
		...
		var defaultNumber = existingReports.map((r) => r.number).sort().reverse()[0]
		defaultNumber = `${parseInt(defaultNumber) + 1}`;
		const questions = [
			{
				name: 'number',
				type: 'input',
