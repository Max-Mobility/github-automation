...
module.exports = {
  selectTest: async function(tests) {
  ...
  performTest: async function() {
    ...
    async function addMethod() {
      const methodAnswer = await inquirer.prompt(method_questions);
    ...
    const resultsAnswer = await inquirer.prompt(results_questions);
  ...
  makeTestReport: async function(existingReports, requirements) {
    ...
    var addTest = async () => {
      const testAnswer = await inquirer.prompt(test_questions);
      if (testAnswer.addTest.indexOf('YES') > -1) {
        const test = await this.performTest();
    ...
    const notesAnswer = await inquirer.prompt(notes_questions);
